...
- **Rate limit**: only `establishment_register_view` (both `/api/auth/register-establishment/` and `/api/mobile/sanitation/register-establishment/`), 5/hour per client, via a `ScopedRateThrottle` subclass with a fixed scope. Counts are kept in a `DatabaseCache` alias `throttle` (table `api_throttle_cache`, created by `python manage.py createcachetable`, added to `build.sh`) so all server processes share them. `NUM_PROXIES = 1`. Login is not throttled (a test pins this).
...
- **Not verified / open**: not merged, not deployed, **a new APK is required** for staff. `NUM_PROXIES = 1` assumes one proxy hop on Render; if Render adds another hop every client would share one bucket — check after deploy. `createcachetable` runs only if `build.sh` is the Render build command. No authenticated browser or device check was performed.
