...
Exactly 5 errors + 5 failures across the 30-test suite (`manage.py test api --keepdb`), all pre-dating any notification/security work:
...
- **Tourism app real staff authentication**: Mirror of what was done for the sanitation app (login UI, token persistence, role verification) — required before touching `mobile_tourist_record_check_in`, `mobile_tourist_record_lookup`, and `mobile_tourist_record_history`.
- **CRITICAL Tourist PII Data Leak**: `mobile_tourist_record_lookup` and `mobile_tourist_record_history` currently return full tourist PII (name, email, phone, demographics) to ANY unauthenticated caller. This is an active data leak on public endpoints and represents the highest priority once tourism auth exists.
